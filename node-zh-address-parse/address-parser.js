 ...
 * @constructor
 */
const AddressParser = (address, options) => {
  const {type = 0, textFilter = [], nameMaxLength = 4} = typeof options
  === 'object' ? options : (typeof options === 'number' ? {type: options} : {})
  ...
  const parseFields = options.parseFields;
  address = cleanAddress(address, textFilter)
  logger.info(`清洗地址：${address}`)

  // 识别手机号
  if (parseFields.phoneNo) {
  ...
  if (parseFields.name) {
    if (detail && detail.length > 0) {
      ...
      const index = copyDetail.findIndex(
          item => judgeFragmentIsName(item, nameMaxLength))
      let name = ''
      if (index !== -1) {
        name = copyDetail[index]
      } else if (copyDetail[0].length <= nameMaxLength
          && /[\u4E00-\u9FA5]/.test(
              copyDetail[0])) {
        name = copyDetail[0]
      }
  ...
  logger.info(`解析结果：${JSON.stringify(parseResult)}`)
...
const parseRegionWithRegexp = (fragment, hasParseResult) => {
  ...
  if (province.length === 0) {
    for (let i = 1; i < fragment.length; i++) {
      ...
      const regexProvince = new RegExp(`\{\"code\":\"[0-9]{1,6}\",\"name\":\"${str}[\u4E00-\u9FA5]*?\"}`,
          'g'
      )
  ...
  if (city.length === 0) {
    for (let i = 1; i < fragment.length; i++) {
      const str = fragment.substring(0, i + 1)
      const regexCity = new RegExp(`\{\"code\":\"[0-9]{1,6}\",\"name\":\"${str}[\u4E00-\u9FA5]*?\",\"provinceCode\":\"${province[0] ? `${province[0].code}` : '[0-9]{1,6}'}\"\}`,
          'g')
    ...
    if (city[0]) {
      ...
      fragment = fragment.replace(new RegExp(matchStr, 'g'), '')
      if (province.length === 0) {
        const regexProvince = new RegExp(`\{\"code\":\"${provinceCode}\",\"name\":\"[\u4E00-\u9FA5]+?\"}`,
            'g')
        const matchProvince = provinceString.match(regexProvince)
  ...
  if (area.length === 0) {
    for (let i = 1; i < fragment.length; i++) {
      const str = fragment.substring(0, i + 1)
      const regexArea = new RegExp(`\{\"code\":\"[0-9]{1,6}\",\"name\":\"${str}[\u4E00-\u9FA5]*?\",\"cityCode\":\"${city[0] ? city[0].code : '[0-9]{1,6}'}\",\"provinceCode\":\"${province[0] ? `${province[0].code}` : '[0-9]{1,6}'}\"\}`,
          'g')
    ...
    if (area[0]) {
      ...
      console.log("area/fragment", fragment)
      fragment = fragment.replace(matchStr, '')
      console.log("area/fragment/replace", fragment)
      if (province.length === 0) {
        const regexProvince = new RegExp(`\{\"code\":\"${provinceCode}\",\"name\":\"[\u4E00-\u9FA5]+?\"}`,
            'g')
        const matchProvince = provinceString.match(regexProvince)
      ...
      if (city.length === 0) {
        const regexCity = new RegExp(`\{\"code\":\"${cityCode}\",\"name\":\"[\u4E00-\u9FA5]+?\",\"provinceCode\":\"${provinceCode}\"\}`,
            'g')
        const matchCity = cityString.match(regexCity)
  ...
  if (street.length === 0) {
    ...
    for (let i = 1; i < fragment.length; i++) {
      const str = fragment.substring(0, i + 1)
      const regexStreet = new RegExp(`\{\"code\":\"[0-9]{1,9}\",\"name\":\"${str}[\u4E00-\u9FA5]*?\",\"areaCode\":\"${area[0] ? area[0].code : '[0-9]{1,6}'}\",\"cityCode\":\"${city[0] ? city[0].code : '[0-9]{1,6}'}\",\"provinceCode\":\"${province[0] ? `${province[0].code}` : '[0-9]{1,6}'}\"\}`,
          'g')
    ...
    if (street[0]) {
      ...
      if (area.length === 0) {
        const regexArea = new RegExp(`\{\"code\":\"${areaCode}\",\"name\":\"[\u4E00-\u9FA5]+?\",\"cityCode\":\"${cityCode}\",\"provinceCode\":\"${provinceCode}\"\}`,
            'g')
        const matchArea = areaString.match(regexArea)
        if (matchArea) {
...
const judgeFragmentIsName = (fragment, nameMaxLength) => {
  ...
  const nameCall = ['先生', '小姐', '同志', '哥哥', '姐姐', '妹妹', '弟弟', '妈妈', '爸爸', '爷爷',
    '奶奶', '姑姑', '舅舅']
